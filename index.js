...
app.get("/getCategory", async (req, res) => {
  ...
  const results = await query(
    conn,
    "SELECT * FROM catalog where catalog.status = 1"
  ).catch(console.log);
  res.send(results);
});
...
app.get("/Category/:idcate", async (req, res) => {
  ...
  const results = await query(
    conn,
    `select * from product where product.idCat = ${id} and showhide = 1`
  ).catch(console.log);
  res.send(results);
});
...
app.get("/infoKH/:idUser", async (req, res) => {
  ...
  const results = await query(
    conn,
    `SELECT * FROM user where user.idUser = ` + id
  ).catch(console.log);
  res.send(results);
});
...
app.post("/searchProduct", async (req, res) => {
  ...
  const results = await query(
    conn,
    "select * from product where product.nameProduct like '%" +
      req.body.search +
      "%' and product.idCat = " +
      req.body.idcate +
      " order by product." +
...
app.post("/inputUser", async (req, res) => {
  ...
  const results = await query(
    conn,
    `INSERT INTO user(ho, ten, email, password, phone, address, tinh, huyen) VALUES('${req.body.ho}','${req.body.ten}','${req.body.email}','${req.body.pass}','${req.body.sdt}','${req.body.diachi}','${req.body.tinh}','${req.body.huyen}')`
  ).catch(console.log);
  res.send(results);
});
...
app.post("/searchsdt", async (req, res) => {
  ...
  const results = await query(
    conn,
    `SELECT * FROM user WHERE user.phone = '${req.body.sdt}'`
  ).catch(console.log);
  res.send(results);
});
...
app.get("/getShoppingCard/:lstSP", async (req, res) => {
  ...
  const results = await query(
    conn,
    "select * from product where idProduct in (" + req.params.lstSP + ")"
  ).catch(console.log);
  res.send(results);
});
...
app.post("/updateKH/:idUser", async (req, res) => {
  ...
  const results = await query(
    conn,
    `UPDATE user SET ho = '${req.body.ho}', ten = '${req.body.ten}', email = '${req.body.email}', password = '${req.body.pass}' WHERE idUser = ` +
      id
  ).catch(console.log);
  res.send(results);
});
app.post("/updateKH3/:idUser", async (req, res) => {
  ...
  const results = await query(
    conn,
    `UPDATE user SET tinh = '${req.body.tinh}', huyen = '${req.body.huyen}', address = '${req.body.diachi}' WHERE idUser = ` +
      id
  ).catch(console.log);
  res.send(results);
});
...
app.post("/buyProduct", async (req, res) => {
  ...
  const data1 = await query(
    conn,
    "INSERT INTO hoadon(idKH) VALUES ('" + MaKh + "')"
  ).catch(console.log);

  const data2 = await query(
    conn,
    "SELECT idHD FROM hoadon ORDER BY idHD DESC LIMIT 1"
  ).catch(console.log);
  ...
  for (var i = 0; i < lstSP.length; i++) {
    ...
    await query(
      conn,
      "UPDATE product SET viewss = viewss + ? WHERE idProduct = ?",
...
app.post("/searchMGG", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await query(conn, "SELECT * FROM giamgia WHERE code = ?", [
      req.body.code,
    ]).catch(console.log);
...
app.get("/hoadon/:idUser", async (req, res) => {
  ...
  const results = await query(
    conn,
    `SELECT * FROM hoadon WHERE hoadon.idKH = '${req.params.idUser}'`
  ).catch(console.log);
  res.send(results);
});
...
app.post("/searchDanhmuc", async (req, res) => {
  ...
  const results = await query(
    conn,
    `SELECT * FROM catalog WHERE catalog.nameCat = '${req.body.namecat}'`
  ).catch(console.log);
  res.send(results);
});
...
app.post("/inputDanhmuc", async (req, res) => {
  ...
  const results = await query(
    conn,
    `INSERT INTO catalog(nameCat) VALUES('${req.body.namecat}')`
  ).catch(console.log);
  res.send(results);
});
...
app.post("/deleteDM/:idCat", async (req, res) => {
  ...
  const results = await query(
    conn,
    `DELETE FROM catalog WHERE idCat = ` + id
  ).catch(console.log);
  res.send(results);
});
...
app.post("/updateDanhmuc/:idCat", async (req, res) => {
  ...
  const results = await query(
    conn,
    `UPDATE catalog SET nameCat = '${req.body.namecat}', status = '${req.body.selectTT}' WHERE idCat = ` +
      id
  ).catch(console.log);
  res.send(results);
});
...
app.get("/infoDM/:idCat", async (req, res) => {
  ...
  const results = await query(
    conn,
    `SELECT * FROM catalog where catalog.idCat = ` + ids
  ).catch(console.log);
  res.send(results);
});
...
app.get("/CategoryADM/:idCat", async (req, res) => {
  ...
  const results = await query(
    conn,
    `select * from product where product.idCat = ` + idp
  ).catch(console.log);
  res.send(results);
});
// Upload image
...
app.post("/upload", (req, res, next) => {
  upload(req, res, async function (err) {
    if (err) {
      ...
      const results = await query(
        conn,
        `insert into product(nameProduct, slug, description, imgProduct, showhide, idCat, viewss, priceProduct) values('${req.body.nameSP}','${req.body.maSP}','${req.body.motaSP}','${req.file.filename}','${req.body.shSP}','${req.body.idcatSP}','${req.body.viewSP}','${req.body.giaSP}')`
      ).catch(console.log);
  ...
  upload(req, res, async function (err) {
    if (err) {
      ...
      if (!req.file) {
        const results = await query(
          conn,
          `UPDATE product SET nameProduct = '${req.body.nameSP}', slug = '${req.body.maSP}', description = '${req.body.motaSP}', showhide = '${req.body.shSP}', idCat = '${req.body.idcatSP}', viewss = '${req.body.viewSP}', priceProduct = '${req.body.giaSP}' WHERE idProduct = ` +
            idu
        ).catch(console.log);
        ...
        const results1 = await query(
          conn,
          `UPDATE product SET nameProduct = '${req.body.nameSP}', slug = '${req.body.maSP}', description = '${req.body.motaSP}', imgProduct = '${req.file.filename}', showhide = '${req.body.shSP}', idCat = '${req.body.idcatSP}', viewss = '${req.body.viewSP}', priceProduct = '${req.body.giaSP}' WHERE idProduct = ` +
            idu
        ).catch(console.log);
...
app.post("/deleteHD/:idHD", async (req, res) => {
  ...
  const results = await query(
    conn,
    `DELETE FROM hoadon WHERE idHD = ` + idh
  ).catch(console.log);
  res.send(results);
});
app.get("/showCTHD/:idHD", async (req, res) => {
  ...
  const results = await query(
    conn,
    "SELECT * FROM chitiethoadon WHERE idHD = ?",
    [id]
  ).catch(console.log);
  res.send(results);
});
app.get("/showCTHDa/:idHD", async (req, res) => {
  ...
  const results = await query(
    conn,
    "SELECT * from chitiethoadon as cthd join hoadon as hd ON cthd.idHD = hd.idHD join product as pr ON cthd.idSP = pr.idProduct join user as us ON hd.idKH = us.idUser where hd.idHD =" +
      id
  ).catch(console.log);
  res.send(results);
});
...
app.post("/uptt/:idHD", async (req, res) => {
  ...
  const results = await query(
    conn,
    `UPDATE hoadon SET tinhtrang = '${req.body.tt}' where idHD =` + id
  ).catch(console.log);
  res.send(results);
});
...
app.get("/SPnoibat", async (req, res) => {
  ...
  const results = await query(
    conn,
    "SELECT * from product ORDER BY viewss DESC LIMIT 4"
  ).catch(console.log);
  res.send(results);
});
...
app.get("/SPmoi", async (req, res) => {
  ...
  const results = await query(
    conn,
    "SELECT * FROM product ORDER BY dateUpdate DESC LIMIT 4"
  ).catch(console.log);
  res.send(results);
});
...
app.post("/updateKH2/:idUser", async (req, res) => {
  ...
  const results = await query(
    conn,
    `UPDATE user SET ho = '${req.body.ho}', ten = '${req.body.ten}', email = '${req.body.email}', phone = '${req.body.sdt}', tinh = '${req.body.tinh}', huyen = '${req.body.huyen}', address = '${req.body.diachi}', password = '${req.body.pass}' where idUser =` +
      id
  ).catch(console.log);
  res.send(results);
});
...
app.post("/showttHD/:idHD", async (req, res) => {
  ...
  const results = await query(
    conn,
    "SELECT * FROM hoadon WHERE idHD = ?",
    [id] // Giá trị id được truyền vào dưới dạng mảng để tránh SQL injection
  ).catch(console.log);
  if (results.length > 0) {
    res.send(results[0]); // Chỉ trả về kết quả đầu tiên nếu có
  } else {
    res.send("Không tìm thấy hóa đơn"); // Trả về thông báo nếu không tìm thấy hóa đơn
  }
});
...
app.post("/inputUser2", async (req, res) => {
  ...
  const results = await query(
    conn,
    `INSERT INTO user(ho, ten, phone, address, tinh, huyen) VALUES('${req.body.ho}','${req.body.ten}','${req.body.sdt}','${req.body.diachi}','${req.body.tinh}','${req.body.huyen}')`
  ).catch(console.log);
  res.send(results);
});
...
app.post("/searchemail", async (req, res) => {
  ...
  const results = await query(
    conn,
    "select * from user where user.email like '%" + req.body.email + "%'"
  ).catch(console.log);
  res.send(results);
});
...
app.post("/searchADMsdt", async (req, res) => {
  ...
  const results = await query(
    conn,
    "select * from user where user.phone like '%" +
      req.body.search +
      "%' and isAdmin = 0"
  ).catch(console.log);
  res.send(results);
...
app.post("/comment/:idProduct", async (req, res) => {
  ...
  const results = await query(
    conn,
    `insert into comment(name, email, rating, content, idProduct) values('${req.body.ten}','${req.body.email}','${req.body.rating}', '${req.body.cmt}','${id}')`
  ).catch(console.log);
  res.send(results);
});
...
app.get("/showcomment/:idProduct", async (req, res) => {
  ...
  const results = await query(
    conn,
    `select * from comment where idProduct =` + id
  ).catch(console.log);
  res.send(results);
});
...
app.post("/checkUser", async function (req, res) {
  ...
  const results = await query(
    conn,
    `select * from user where idUser = ${req.body.id} and isAdmin = 1`
  ).catch(console.log);
  res.send(results);
});
